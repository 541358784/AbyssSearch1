...
public class ColliderCheckController : Singleton<ColliderCheckController>
{
    ...
    public Dictionary<ColliderTargetType, List<ICollider>> TriggerIColliders;
    public HashSet<(ICollider, ICollider)> IColliderDeduplicationSet; //碰撞去重表

    public List<ColliderTargetType> TriggerType = new List<ColliderTargetType>()
    {
        ColliderTargetType.Wall,
        ColliderTargetType.PlayerBullet,
        ColliderTargetType.Player
    }; //用来遍历的触发器类型，先判断墙体撞击，有可能导致位移的碰撞优先处理,玩家的子弹数量比较可控
    ...
    public void Init()
    {
        ...
        TriggerIColliders = new Dictionary<ColliderTargetType, List<ICollider>>();
        for (var i = 0; i < TriggerType.Count; i++)
        {
            TriggerIColliders.Add(TriggerType[i], new List<ICollider>());
        }
    ...
    {
        var minPoint = ICollider.ColliderSurroundRect.min;
        var maxPoint = ICollider.ColliderSurroundRect.max;
    ...
    public void Register(ICollider ICollider)
    {
        ...
        if (TriggerType.Contains(ICollider.Type))
        {
            TriggerIColliders[ICollider.Type].Add(ICollider);
        }
    }
    ...
    public void UnRegister(ICollider ICollider)
    {
        ...
        if (TriggerType.Contains(ICollider.Type))
        {
            TriggerIColliders[ICollider.Type].Remove(ICollider);
        }
    }
    ...
    public void CheckICollider() //检测碰撞
    {
        ...
        foreach (var pair in TriggerIColliders) //按碰撞体类型顺序检测碰撞,优先墙体，墙体可能导致位移，位移会对碰撞结果产生影响
        {
            foreach (var triggerICollider in pair.Value)//遍历每个触发器
            {
                    ...
                    for (var y = rect.minY; y <= rect.maxY; y++)
                    {
                        foreach (var ICollider in Grids[x, y].GetContains(triggerTypes)) //遍历格子内每个类型正确的碰撞体
                        {
                            if (ICollider == triggerICollider)
                                continue;
                            if (ColliderShapeHandler.Instance.IsOverlap(ICollider, triggerICollider))//是否重叠
                            {
