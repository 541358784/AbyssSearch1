...
public class ApplicationExceptionHandler : MonoBehaviour
{
  //  private int _maxExceptionCount = 200;
    ...
    public List<string> logFilterOnUI = new List<string>()
        {"Track", "Assets", "Lock", "N", "D", "E", "E&S", "ALL", "All&S"};

    public int selectIndex = 4;

    public List<string> logFilters = new List<string>() {"^~^", "[TEngine]"};
    public List<string> logExcludeFilters = new List<string>() {"Cache verify file"};


    private static GUIStyle style = new GUIStyle();
    private static Texture2D texture = null;

    private static int maxItemToShow = 100;
    ...
    private void HandleException(string condition, string stackTrace, LogType type)
    {
        ...
        {
            if (type == LogType.Exception && !condition.Contains("TaskCanceledException"))
            {
                {
            ...
            for (var i = 0; i < logExcludeFilters.Count; i++)
            {
                if (condition.Contains(logExcludeFilters[i]))
                {
                    return;
                }
            }
    ...
    void OnGUI()
    {
        ...
        {
            GUILayout.BeginVertical(Get(new Color(0.0f, 0.0f, 0.0f, 0.8f)), GUILayout.Width(Screen.width),
        ...
        var ratio = Math.Max(Screen.width, Screen.height) /  DesignResolution.matchResolution.x;
        ...
        if (_showExceptionView)
        {
            ...
            if (GUILayout.Button("复制", customButtonStyle, GUILayout.Width(buttonSizeWidth),
                GUILayout.Height(buttonSizeHeight)))
            {
                var stringBuffer = "";
            ...
            if (GUILayout.Button("清除", customButtonStyle, GUILayout.Width(buttonSizeWidth),
                GUILayout.Height(buttonSizeHeight)))
        ...
        if (_showExceptionView)
        {
            GUI.skin.verticalScrollbar.fixedWidth = DesignResolution.matchResolution.x * 0.03f;
            GUI.skin.verticalScrollbarDownButton.fixedWidth =  DesignResolution.matchResolution.x * 0.03f;
            ;
            GUI.skin.verticalScrollbarThumb.fixedWidth =  DesignResolution.matchResolution.x * 0.03f;
            ;
            GUI.skin.verticalScrollbarUpButton.fixedWidth =  DesignResolution.matchResolution.x * 0.03f;
            ;
            //   GUI.skin.scrollView = scrollViewStyle;

            GUILayout.BeginVertical();
            selectIndex = GUILayout.Toolbar(selectIndex, logFilterOnUI.ToArray(), customButtonStyle,
                GUILayout.Height(buttonSizeHeight));

            _scrollPosition = GUILayout.BeginScrollView(
                _scrollPosition, GUILayout.Width(Screen.width), GUILayout.Height(Screen.height - buttonSizeHeight * 3));

            var currentItemCount = 0;
            var startIndex = maxItemToShow * currentPage;
            var showCount = 0;
            foreach (var item in _collectExceptions)
            {
                if (MatchCurrentFilter(item.Item1, item.Item2))
                {
                    ...
                    if(currentItemCount < startIndex)
                        continue;
                    
                    if (currentItemCount >= maxItemToShow + startIndex)
                    {
                        break;
                    }
        ...
        {
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Pre", customButtonStyle, GUILayout.Width(buttonSizeWidth),
                GUILayout.Height(buttonSizeHeight)))
            {
                currentPage = Math.Max(0, currentPage - 1);
            }

            if (GUILayout.Button("Next", customButtonStyle, GUILayout.Width(buttonSizeWidth),
                GUILayout.Height(buttonSizeHeight)))
            {
                currentPage++;
            }
    ...
    private bool MatchCurrentFilter(string logItem, string logTag)
    {
        switch (selectIndex)
        {
            ...
            case 1:
                return logTag == "Log" && logItem.Contains("[YooAssets]") && !logItem.Contains("Cache verify file");
            case 2:
                return logTag == "Log" && logItem.Contains("BlockingUserClick");
            case 3:
                return logTag == "Log" && logItem.Contains("[NetWork]");
            case 4:
                return logTag == "Log" && !logItem.Contains("TrackerPoint:") && !logItem.Contains("[YooAssets]") &&
                       !logItem.Contains("[NetWork]");
            case 5:
    ...
    private void ShowBlockUI()
    {
        ...
        {
            blockingUserClickUI = GameObject.Find("Root/UIRoot/UICanvas").transform
                .Find("ExceptionHandlerUI");
            if (blockingUserClickUI == null)
            {
                var gameObject = new GameObject("ExceptionHandlerUI");
                gameObject.transform.SetParent(GameObject.Find("Root/UIRoot/UICanvas").transform,
                    false);
